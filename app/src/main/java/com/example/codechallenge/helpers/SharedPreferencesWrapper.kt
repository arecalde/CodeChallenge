...
object SharedPreferencesWrapper {
    ...
    fun setBatteryInfo(batteryInfo: BatteryInfo) {
        prefs.edit()?.putString(BATTERY_STATE_KEY, gson.toJson(batteryInfo))?.apply()
    }

    fun getBatteryInfo(): BatteryInfo? {
        val json = prefs.getString(BATTERY_STATE_KEY, "")
        return gson.fromJson(json, BatteryInfo::class.java)
    }

    fun setLocation(location: Location) {
        prefs.edit()?.putString(LOCATION_KEY, gson.toJson(location))?.apply()
    }
    ...
    fun getAppsList(): Set<String>? {
        return prefs.getStringSet(APPS_LIST_KEY, null)
    }
